...
import {ModuleConfig, SLOT_INDEX_TOKEN_BALANCE, SLOT_INDEX_DELEGATE} from "src/module/governance-pool/ModuleConfig.sol";
    ...
    ModuleConfig
{
    ...
    function bid(
        ...
        NounsDAOStorageV2.ProposalCondensed memory eProp = NounsGovernanceV2(
            _cfg.externalDAO
        ).proposals(_pId);
    ...
    function castVote(uint256 _pId) external {
        ...
        b.executed = true;
        b.remainingVotes = NounsToken(_cfg.externalToken).getPriorVotes(
            _cfg.base,
            _voteSnapshotBlock(b, _pId)
        );
        ...
        uint256 refund = _gasRefundWithTipAndCap(
            startGas,
            b.remainingAmount,
            _cfg.maxBaseFeeRefund,
            _cfg.tip
        );
    ...
    function withdraw(
        ...
        if (msg.sender != _tokenOwner) {
            ...
            bool isDelegate = dr.checkDelegateForContract(
                msg.sender,
                _tokenOwner,
                address(this)
            );
        ...
        for (uint256 i = 0; i < len; ) {
            ...
            Fact[] memory facts = IBatchProver(_prover).proveBatch{
                value: _fee[i]
            }(_proofBatches[i], false);
            ...
            uint256 ownerShare = (nounsBalanceVal * b.remainingAmount) /
                b.remainingVotes;
