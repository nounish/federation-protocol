...
contract TestPoolManager is Test {
  ...
  function testSetUseStartBlock() public {
    ...
    ModuleConfig.Config memory _pcfg = ModuleConfig(address(pool)).getConfig();
    assertEq(_pcfg.useStartBlockFromPropId, 1);
  }

  function testSetAddresses() public {
    bytes4 s = ModuleConfig.setAddresses.selector;
    bytes memory callData = abi.encodeWithSelector(s, address(0), address(0), address(0));
    ...
    callData = abi.encodeWithSelector(s, address(1), address(1), address(1));
    vm.prank(owner);
    manager.execute(callData);
